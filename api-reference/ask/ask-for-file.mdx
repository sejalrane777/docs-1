...
<ParamField path="error_msg" type="str" optional>
  Error message will display when the user doesn't provide the expected output.
</ParamField>
...
you can also display an error message when the user doesn't provide the expected output.
```python Ask for a python file
import chainlit as cl

file = await cl.AskFileMessage(
        content="Please upload a python file to begin!", accept={"text/plain": [".py"],error_msg='Files size should be less than 2 MB}
      ).send()
```
